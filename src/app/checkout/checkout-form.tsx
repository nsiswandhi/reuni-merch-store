...
// shippingFlatRate comes from the server (AdminSettings) via page.tsx — the
// client has no direct DB access, and the buyer needs to see the real total
// (subtotal + ongkir) before submitting, not just after the order is
// created, since delivery method is already known at this point.
export function CheckoutForm({ shippingFlatRate }: { shippingFlatRate: number }) {
    ...
    <main className="mx-auto max-w-2xl px-4 py-8">
      ...
      <form action={formAction} className="flex flex-col gap-4">
        ...
        <div className="rounded border border-gray-200 p-3">
          ...
          {deliveryMethod === "SHIPPING" && <p>Ongkir: {formatRupiah(shippingCost)}</p>}
          <p className="font-semibold">Total: {formatRupiah(total)}</p>
          {isPreorder && (
            <p className="mt-1 text-sm text-gray-500">
              Ini estimasi total (sudah termasuk ongkir) — tagihan resmi dikirim lewat email begitu kuota preorder terpenuhi.
            </p>
          )}
        </div>
